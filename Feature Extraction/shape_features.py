	...
	for contour in contours:
		...
		if (area >= 100000 and area <= 900000): # (4-9, top) (1-9, bot,sides)
			...
			for i in range(len(contour_list)):
				...
				# Hu Moments
				## It is used to describe, characterize, and quantify the shape of an object in an image.
    			# hu_moments = cv2.HuMoments(cv2.moments(cimg)).flatten()
    			# print hu_moments
    			...
    			for cnt in contour_list:
	    			...
	    			cv2.drawContours(clone1,[approx],-1,(0,255,0),10)
	    			# print approx

    			## Convex Hull
    			for cnt in contour_list:
				    hull = cv2.convexHull(cnt)
				    cv2.drawContours(clone2, [hull], -1, (0, 0, 255), 10)
				    # print hull
